...
1. **`gh` CLI** — fallback when MCP tools can't do the operation
...
1. Never close issues without validated success; if validation isn't possible, get user confirmation first
1. Always post agent results (validation, QA, reviews) as issue comments, never edit issue body
