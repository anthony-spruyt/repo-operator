...
1. **Official docs URL known?** → WebFetch (allowed domains only)
1. **All above failed?** → WebSearch (state why others failed first)

## When Context7 Doesn't Have the Library

1. Check GitHub → `gh issue list --repo org/repo --search "topic"`
1. Fetch README → WebFetch `raw.githubusercontent.com/.../README.md`
1. Only then → WebSearch, explaining: "Context7 and GitHub don't have X, using web search"
...
1. **Search for real implementations** → `gh search code "the pattern" --language <lang>`
1. **Check recent issues** → `gh search issues "error message" --repo org/repo`
1. **Fetch actual source** → WebFetch `raw.githubusercontent.com/.../src/file.ts`
