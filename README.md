...
| `repo-operator-release-bot[bot]` | 4745999 | `RELEASE_PLEASE_APP_*`, synced to container-images, spruyt-labs, SunGather, xfg | Opens release PRs, creates tags and releases     | `tag-rules` `always` on xfg              |
...
`pr-rules` only exists on `protected-main-branch` repos. The release bot runs release-please in all four release repos; only xfg gives it a bypass, for moving the floating `vN` tag. Mergify merge protections trust PRs authored by `repo-operator-release-bot[bot]` (`release-please`, `megalinter-refresh`); that is an author match, not a ruleset bypass.
